...
import lirrStops from '@/lib/data/lirr-stops.json';
...
const asStopMap = lirrStops as unknown as Record<string, StopRecord>;
...
export async function GET(request: Request) {
	...
	inFlight = (async () => {
		const urlsRaw = process.env.LIRR_GTFS_RT_URLS ?? process.env.LIRR_GTFS_RT_URL;

		if (!urlsRaw) {
			return {
				payload: { ok: false, updatedAt: Date.now(), activeStops: [], error: 'Missing LIRR_GTFS_RT_URLS' },
				status: 500,
			};
		}
		...
		const apiKey = process.env.LIRR_API_KEY ?? process.env.MTA_API_KEY;
